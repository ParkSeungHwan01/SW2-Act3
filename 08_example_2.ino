...
void loop() {
  ...
  if ((distance < _DIST_MIN) || (distance > _DIST_MAX)) {
      ...
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (_DIST_MIN <= distance && distance < 200.0){
      digitalWrite(PIN_LED, 0);
      L = 255-(distance-100)*(255/100); 
      L = (int)L;
      analogWrite(PIN_LED, L);
  } else if (200.0 <= distance && distance <= _DIST_MAX) {    // In desired Range
      digitalWrite(PIN_LED, 0);
      L = (distance-200)*(255/100);       // LED ON  
      L = (int)L;
      analogWrite(PIN_LED, L);
  }
